...
export default async function handler(req, res) {
  ...
  try {
    ...
    const transparencyLevel = fields.transparencyLevel?.[0] || '100';
    ...
    const outputFilename = `transparent_${Date.now()}.png`;
    ...
    // Run Python script for transparency processing
    const pythonScript = path.join(process.cwd(), 'scripts', 'transparent_background.py');
      ...
      pythonProcess.on('close', (code) => {
        if (code !== 0) {
          ...
          return res.status(500).json({ 
            error: 'Transparency processing failed',
            details: errorOutput 
          });
        }

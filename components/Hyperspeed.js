  ...
  useEffect(() => {
    ...
    class App {
      constructor(container, options = {}) {
        ...
        // Initialize composer later to speed up initial render
        this.composer = null;
      ...
      initPasses() {
        // Create composer now that assets are loaded
        if (!this.composer) {
      ...
      loadAssets() {
        ...
        return new Promise(resolve => {
          ...
          const updateProgress = () => {
            loadedItems++;
            const progress = (loadedItems / totalItems) * 90; // Reserve 10% for completion
      ...
      initBasic() {
        // Initialize basic animation without post-processing effects
      ...
      init() {
        // Only initialize post-processing effects since basic animation is already running
        this.initPasses();
        // The tick() is already running from initBasic(), no need to start it again
      }
      ...
      render(delta) {
        // Use basic renderer during loading, enhanced composer after assets load
        if (this.composer && this.assets.smaa && this.assets.smaa.search && this.assets.smaa.area) {
          this.composer.render(delta);
        } else {
          // Basic rendering without post-processing effects
    ...
    (function () {
      ...
      // Start basic animation immediately
      myApp.initBasic();
      
      // Load assets and then enhance the animation
      myApp.loadAssets().then(() => {
        myApp.init();
        myApp.initPasses(); // Initialize post-processing effects
      });
    })();
      ...
      {isLoading && (
        <div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center z-10">
          <div className="text-center">
            ...
            <h3 className="text-xl font-semibold text-white mb-2">Initializing Hyperspeed</h3>
            <p className="text-gray-300 mb-4">Preparing the ultimate animation experience...</p>
